    ...
    return new Promise((resolve, reject) => {
        fetch('https://jagokata.com/kata-bijak/kata-' + input.replace(/\s/g, '_') + '.html?page=1')
            ...
            .then(res => {
                ...
                data.splice(2, 1)
                if (data.length == 0) return resolve({ creator: 'ANIRUDH', status: false })
                resolve({ creator: 'ANIRUDH', status: true, data })
            }).catch(reject)
    })
}
...
function joox(query) {
    return new Promise((resolve, reject) => {
        ...
        axios.get('http://api.joox.com/web-fcgi-bin//web_search?lang=id&country=ind&type=0&search_input=' + query + '&pn=1&sin=0&ein=29&_=' + time)
            .then(({ data }) => {
                let result = []
                let hasil = []
                let promoses = []
...
function facebook(url) {
    return new Promise(async (resolve, reject) => {
        ...
        axios.post(host + '/system/action.php', qs.stringify(form.data), { headers: is.headers })
            .then(({ data }) => {
                if (data.links.lenght == 0) return resolve({ creator: '@ANIRUDH', status: false })
                resolve({ creator: '@ANIRUDH', status: true, data: data.links })
            })
    })
}
